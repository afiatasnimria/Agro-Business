...
<head>
  ...
  <title>Agro Management — Expenses</title>
  <style>
    /* ----- Base ----- */
    :root{
...
<body>
  <div class="app">
    ...
    <main class="main">
      <header class="main-header">
        <div>
          <h1 style="margin:0;font-size:20px">Expenses</h1>
          <div class="small muted">Track business expenses</div>
        </div>
        <div class="top-actions">
          <input placeholder="Search expenses..." id="globalSearch" />
        </div>
      </header>

      <!-- EXPENSES PAGE -->
      <div class="card">
        <table id="expensesTable"><thead><tr><th>Title</th><th>Category</th><th>Amount</th><th>Date</th></tr></thead><tbody></tbody></table>
      </div>
    </main>
  </div>
  ...
  <script>
// ===== Demo data =====
const demoExpenses = [{title:'Electricity', category:'Utilities', amount:1200, date:'2025-11-01'}];
...
function renderExpenses(){
  ...
  demoExpenses.forEach(e=>{
    ...
    tr.innerHTML = `<td>${e.title}</td><td>${e.category}</td><td>৳ ${e.amount}</td><td>${e.date}</td>`;
...
$('#globalSearch').addEventListener('input',e=>{
  const q = e.target.value.toLowerCase(); if(!q) return; const found = demoExpenses.find(ex=>ex.title.toLowerCase().includes(q)); if(found){ alert('Found expense: '+found.title) }
});
